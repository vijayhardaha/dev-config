...
- **One preset entry per technology.** Each preset folder exposes either a default export (eslint, prettier, commitlint, stylelint), a named export (`smacssOrder` for gulp-smacss, `createSitemapConfig` for next-sitemap), or a JSON file (tsconfig, jsconfig) plus, where useful, a `createConfig` function for options. next-sitemap has no default export: `siteUrl` is required, so an import-time default config could not exist without silently baking in a placeholder URL. The eslint preset additionally has a `recipes/` subdir for opt-in fragments.

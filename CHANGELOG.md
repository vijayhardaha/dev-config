...
- Streamline the repository ignore files. `.gitignore` root-anchors the build and log directories (`/out/`, `/build/`, `/dist/`, `/storage/logs/`, `/log/`, `/logs/`) so they only match at the repo root, regroups the Nuxt, PostCSS, and GitHub CLI entries under their own headers, alphabetizes the macOS block, prunes the AI-tool list to the tools in use, and replaces the sitemap and robots patterns with explicit globs (`**/public/robots.txt`, `**/public/sitemap*.xml`, `sitemap*.xml`). `.prettierignore` is slimmed to lockfiles, CI YAML, asset directories, and minified files, since Prettier already skips files it cannot parse.
- Bump the `eslint-plugin-jsdoc` dev dependency to `^64.3.1`.
- Enforce the 100% coverage target instead of only reporting it: `vitest.config.mjs` now sets `coverage.thresholds` to 100 across statements, branches, functions, and lines (vitest exits non-zero on any drop), and the release-it `before:init` gate runs `bun run test:coverage` instead of plain `test`, so a coverage regression blocks publishing. The `.release-it.json` `$schema` pin is refreshed to release-it 21.
...
- Scope the Biome presets to JavaScript-based files only: `files.includes` is now an allowlist of JS/TS/JSX globs (`**/*.{js,jsx,mjs,cjs,ts,tsx,mts,cts}`) — with negations for asset output dirs and minified JS — instead of `["**"]` plus a long formatter negation list. Biome no longer processes JSON, CSS, or HTML; the `css` and `html` config sections (including `css.parser.tailwindDirectives` from 2.6.0) are removed. Overrides for languages Biome cannot format (Python, PHP, YAML, Markdown, XML) and the JSON `trailingCommas` setting that never applied are also removed, the JS quote override is collapsed into a top-level `quoteStyle: "single"`, folder ignores drop the trailing `/**` per Biome 2.2+ guidance, and the `._circleci` negation typo is corrected to `.circleci`. Formatting for JSON, CSS, SCSS, HTML, Markdown, YAML, and XML remains with Prettier.
- Correct repository documentation drift: the README release notes now reference the module locations that exist after the 2.5.0 restructure and are ordered newest-first, the previously undocumented `./gulp-smacss` export has a usage example and feature-list entry, the tsconfig/jsconfig `@/*` path-alias inheritance is documented, and AGENTS.md fixes the version note, gulp-smacss export shape, barrel-exclusion list, a phantom ESLint test entry, and the husky optional flag. The root barrel's ESLint re-export JSDoc is typed as `Linter.Config[]`.
